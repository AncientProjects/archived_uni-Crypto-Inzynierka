...
class LSTMSingleOutputModelM3(BaseModel):
    def __init__(self, config):
        super(LSTMSingleOutputModelM3, self).__init__(config)
        self.build_model()

    def build_model(self):
        self.model = tf.keras.models.Sequential()
        # self.model.add(tf.keras.layers.LSTM(20, input_shape=(self.window_size, 1), return_sequences=True))
        self.model.add(tf.keras.layers.LSTM(128, input_shape=(self.window_size, 1), return_sequences=True))
        # self.model.add(tf.keras.layers.Dropout(0.1))
        self.model.add(tf.keras.layers.LSTM(128, input_shape=(self.window_size, 1)))
        # self.model.add(tf.keras.layers.Dropout(0.1))
